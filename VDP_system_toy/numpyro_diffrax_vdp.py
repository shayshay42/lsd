...
def model(batch, hidden_dim=2, z_dim=1, output_dim=3, data_size=3, width_size=64, depth=2, key=jr.PRNGKey(0)):
    ...
    neural_ode = numpyro.module("neuralode", NeuralODE(data_size, width_size, depth, key=model_key))
...
def guide(batch, hidden_dim=2, z_dim=1, output_dim=3):
    ...
    #I want the batch to hold basically the timeseries of multiple samples
    with numpyro.plate("data", batch_dim):
